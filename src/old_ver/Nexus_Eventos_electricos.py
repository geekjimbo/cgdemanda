...
def log_write(New_String):
    ...
    filename = str(datetime.now().strftime('%d%m%Y')+ '_NexusEvent.log')
...
log_write("Inicializacion del Scrip de Bases de Datos de Eventos Electricos para Nexus de CoopeGuanacaste")
for i in data_json["Nexus_PQ"]["archivos"]:
    print i["name"]
...
#            my_unb_list=[]
#            my_unb_list2=[]
            for electrical_variable in data_json["Nexus_PQ"]["variables"]:
...
#                var_adjust = electrical_variable["adjust"]
                ...
                if (DB_var_name == "timestamp"):
                    temp_time = datetime.strptime(var_value, "%d/%m/%Y %I:%M:%S %p")
                    var_value = time.mktime(temp_time.timetuple())
                    #var_value.replace(" ", "")
                    #if var_value == "":
                     #   var_value = 0
                    var_value=1000*float(var_value)
...
#                    var_value= x + mSeconds
                    if (float(var_value) > 300):
                        log_write("INFO: Evento con duracion >5min Encontrado: " +str(var_value) + " Segundos en: " + i["name"]+"." + DB_var_name)
...
#                print my_item
   ...
   # pdb.set_trace()
    ...
    con.close()
    my_dict=[]
print "Terminando el Loop de Barras. Continuando..."
