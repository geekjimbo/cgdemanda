...
#    pdb.set_trace()
    ...
    if (temp_filename == "" or temp_filename == "procesados" or temp_filename == "dir" or temp_filename == "xlsx"):
        print "nothing "
        continue
    temp_path = Data_Base_dir + temp_filename
    if  1 == 2: #(os.path.exists(temp_path)):
...
#                    print "loca"
                ...
                if not(my_local_dict.has_key(column_name)):
                    ...
                    log_write("INFO: Agregando nueva columna a Control de demanda: " + column_name + " con un valor de contador en: " + row_value + " encontrado en el Archivo:" + temp_filename)
    ...
    print my_dict
    subprocess.call(["cp", (ctlfile_dir + 'demanda.txt') ,(ctlfile_dir + 'demanda.txt.old')])
    pickle.dump(my_local_dict , open(ctlfile_dir + 'demanda.txt' , 'wb'))
    my_local_dict= {}
    log_write("INFO: Escribiendo archivo interno de Control de Demanda. Cantidad de lineas: " + str(len(my_dict)))
    con = psycopg2.connect(postgresql_path)
    cur = con.cursor()
    #cur.executemany(query_insert_demanda, my_dict)
    con.commit()
